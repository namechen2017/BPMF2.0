...
class OrgController extends BaseController
{
    ...
    public function getOrgTree(){
        ...
        // 查询出所有的机构
        $rules = $m->where(array('status'=>1))->field('id,name,p_id,introduce,enabled')->order('no asc')->select();
        if($isGetRole == 1){
    ...
    public function editOrg(){
        if (IS_POST) {
            ...
            // 机构节点需要移动
            if($father_id != 0){
                $forg = M('auth_org')->where(array('id'=>$father_id,'status'=>1))->find();
                $edit_data['p_id']         = $father_id;
                $edit_data['relationship'] = $forg['relationship'].",".$org_id;
            }
            $edit = M('auth_org')->where(array('id'=>$org_id,'status'=>1))->save($edit_data);
    ...
    public function DeleteOrg(){
        if (IS_POST) {
            ...
            $sonOrg = M('auth_org')->where(['p_id' => $org_id,'status'=>1])->find();
            if($sonOrg) $this->response(['status' => 103, 'msg' => '有子机构不可直接删除'],'json');

            //$del = M('auth_org')->where(['id' => $org_id])->delete();
            $del = M('auth_org')->where(['id' => $org_id])->setField('status',0);
            if ($del) {
                $this->response(['status' => 100],'json');
            } else {
    ...
    public function searchOrg()
    {
        ...
        $org = M('auth_org')
            ->where("(name LIKE '%$searchText%' OR introduce LIKE '%$searchText%' ) and status = 1")
