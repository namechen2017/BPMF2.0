...
class UcenterController extends RestController
{
    ...
    public function userEdit(){
        if (IS_POST) {
            ...
            $user = M('auth_user')->where(array('id'=>$uid,'status'=>1))->find();
            ...
            $edit = M('auth_user')->where(array('id'=>$uid,'status'=>1))->save($edit_data);
            if ($edit) {
    ...
    public function navManage(){
        ...
        // 通过拥有的权限读导航id
        $navs = $rule->where("id IN ($auths) and status = 1")->field('nav_id')->select();
        foreach($navs as $nv){
            if($nv['nav_id'] != 0){
                $nav_id[] = $nv['nav_id'];
            }
