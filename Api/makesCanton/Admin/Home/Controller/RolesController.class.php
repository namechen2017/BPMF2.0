...
class RolesController extends BaseController{
	...
	public function getRoles(){
		...
		foreach ($sql as $key => $value) {
			...
			$ids = implode(',',$arr);
			if(!empty($ids)){
				$get = $org->field("id,name")->where("id in (".$ids.") and status = 1")->select();
	...
	public function updateRoles(){
		...
		$sql = $role->data($data)->where(array('id'=>$id,'status'=>1))->save();
		...
		if($sql !== 'flase'){
			...
			$arr['status'] = 100;
                        ...
                        $memcacheCache->flushCache('auth_role_field');
		}else{
			$role->rollback();
			$arr['status'] = 101;
			$arr['msg'] = "修改失败";
	...
	public function delRoles(){
		...
		if(empty($query['id'])){
			$role->where("id=%d",array($id))->setField('status',0);
	...
	public function getRule2Role(){
		...
		// 列出所拥有的权限
		$role = M('auth_role');
		$sql = $role->field("permissions")->where(array('id'=>$role_id,'status'=>1))->find();
		$arr = explode(",",$sql['permissions']);
		// 列出所有权限 
		$rule = M('auth_rule');
		$query = $rule->where(array('status'=>1,'enabled'=>1))->field("id,name,p_id")->select();
	...
	public function allotRule2Role(){
		...
		$data['modified_time'] = date('Y-m-d H:i:s',time());
		$sql = $role->data($data)->where(array('id'=>$role_id,'status'=>1,'enabled'=>1))->save();
		if($sql !== 'flase'){
			$arr['status'] = 100;
                        //成功后删除缓存
                        $memcacheCache = new \Think\Product\MemcacheCache();
