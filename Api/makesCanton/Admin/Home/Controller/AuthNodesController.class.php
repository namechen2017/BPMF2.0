...
class AuthNodesController extends BaseController
{
    ...
    public function getRulesTree(){
        ...
        $rules = $m->where(array('enabled'=>$enabled,'status'=>1))->field('id,auth_address,p_id,name')->select();
    ...
    public function insertRule(){
        ...
        if($father_id != 0) {
            if (!preg_match("/^([A-z]+\/[A-z]+)|([A-z]+\/[A-z]+\/[A-z]+)$/", $url)) {
                $this->response(['status' => 102, 'msg' => '有上级示例：“Hello/Word”'], 'json');
            }
        }
        if($father_id == 0){
            if(!preg_match("/^([A-z]+)|([A-z]+\/[A-z]+)$/" , $url)){
        ...
        if($add){
            ...
            $memcacheCache->flushCache('auth_rule_field');
    ...
    public function editRule(){
        ...
        if(!preg_match("/^([A-z]+\/[A-z]+)|([A-z]+\/[A-z]+\/[A-z]+)$/" , $url) && !preg_match("/^([A-z]+)|([A-z]+\/[A-z]+)$/" , $url)){
        ...
        // 编辑
        $edit = $m->where(['id' => $rule_id,'status'=>1])->save($edit_data);
        if($edit){
            //成功后删除缓存
            $memcacheCache = new \Think\Product\MemcacheCache();
            $memcacheCache->flushCache('auth_rule_field');
    ...
    public function deleteRule(){
        ...
        $hasSon = $m->where(['p_id' => $rule_id,'status'=>1])->find();
        if($hasSon) $this->response(['status' => 101,'msg' => '存在子节点']);

        $role = M('auth_role')->where("permissions LIKE '%$rule_id%' and status = 1")->select();
        if($role) $this->response(['status' => 101,'msg' => '该节点（'.$hasSon['name'].'）已被角色组使用']);
        // 删除
        //$del = $m->where(['id' => $rule_id])->delete();
         $del = $m->where(['id' => $rule_id])->setField('status',0);
        if($del){
            //成功后删除缓存
            $memcacheCache = new \Think\Product\MemcacheCache();
            $memcacheCache->flushCache('auth_rule_field');
    ...
    public function getRulesById(){
        ...
        // 查询出所有的机构
        $rule_id = (int)I('rule_id');
        $rule = $m->where(array('id'=>$rule_id,'status'=>1))->find();
